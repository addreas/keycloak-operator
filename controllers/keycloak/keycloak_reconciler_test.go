...
import (
	...
	kcAssert "github.com/addreas/keycloak-operator/test/assert"
...
func TestKeycloakReconciler_Test_Creating_All(t *testing.T) {
	...
	desiredState := reconciler.InnerReconcile(currentState, cr)
	...
	kcAssert.Each(t, desiredState, func(t *testing.T, expected, actual interface{}) {
		assert.IsType(t, expected, actual)
	}, common.GenericCreateAction{})
	kcAssert.FindGenericCreateAction(t, desiredState, model.KeycloakAdminSecret(cr))
	kcAssert.FindGenericCreateAction(t, desiredState, model.PrometheusRule(cr))
	kcAssert.FindGenericCreateAction(t, desiredState, model.ServiceMonitor(cr))
	kcAssert.FindGenericCreateAction(t, desiredState, model.GrafanaDashboard(cr))
	kcAssert.FindGenericCreateAction(t, desiredState, model.DatabaseSecret(cr))
	kcAssert.FindGenericCreateAction(t, desiredState, model.PostgresqlPersistentVolumeClaim(cr))
	kcAssert.FindGenericCreateAction(t, desiredState, model.PostgresqlDeployment(cr, true))
	kcAssert.FindGenericCreateAction(t, desiredState, model.PostgresqlService(cr, model.DatabaseSecret(cr), false))
	kcAssert.FindGenericCreateAction(t, desiredState, model.KeycloakService(cr))
	kcAssert.FindGenericCreateAction(t, desiredState, model.KeycloakDiscoveryService(cr))
	kcAssert.FindGenericCreateAction(t, desiredState, model.KeycloakMonitoringService(cr))
	kcAssert.FindGenericCreateAction(t, desiredState, model.KeycloakProbes(cr))
	kcAssert.FindGenericCreateAction(t, desiredState, model.KeycloakDeployment(cr, model.DatabaseSecret(cr)))
	kcAssert.FindGenericCreateAction(t, desiredState, model.KeycloakRoute(cr))
}
...
func TestKeycloakReconciler_Test_Updating_All(t *testing.T) {
	...
	kcAssert.Each(t, desiredState, func(t *testing.T, expected, actual interface{}) {
		assert.IsType(t, expected, actual)
	}, common.GenericUpdateAction{})
	kcAssert.FindGenericUpdateAction(t, desiredState, model.KeycloakAdminSecret(cr))
	kcAssert.FindGenericUpdateAction(t, desiredState, model.PrometheusRule(cr))
	kcAssert.FindGenericUpdateAction(t, desiredState, model.ServiceMonitor(cr))
	kcAssert.FindGenericUpdateAction(t, desiredState, model.GrafanaDashboard(cr))
	kcAssert.FindGenericUpdateAction(t, desiredState, model.DatabaseSecret(cr))
	kcAssert.FindGenericUpdateAction(t, desiredState, model.PostgresqlPersistentVolumeClaim(cr))
	kcAssert.FindGenericUpdateAction(t, desiredState, model.PostgresqlDeployment(cr, true))
	kcAssert.FindGenericUpdateAction(t, desiredState, model.PostgresqlService(cr, model.DatabaseSecret(cr), false))
	kcAssert.FindGenericUpdateAction(t, desiredState, model.KeycloakService(cr))
	kcAssert.FindGenericUpdateAction(t, desiredState, model.KeycloakDiscoveryService(cr))
	kcAssert.FindGenericUpdateAction(t, desiredState, model.KeycloakMonitoringService(cr))
	kcAssert.FindGenericUpdateAction(t, desiredState, model.KeycloakDeployment(cr, model.DatabaseSecret(cr)))
	kcAssert.FindGenericUpdateAction(t, desiredState, model.KeycloakRoute(cr))
	kcAssert.FindGenericUpdateAction(t, desiredState, model.KeycloakMetricsRoute(cr, model.KeycloakRoute(cr)))
}
...
func TestKeycloakReconciler_Test_Setting_Resources(t *testing.T) {
	...
	keycloakDeployment := kcAssert.FindGenericCreateAction(t, desiredState, model.KeycloakDeployment(cr, model.DatabaseSecret(cr)))
	keycloakContainer := keycloakDeployment.Ref.(*v13.StatefulSet).Spec.Template.Spec.Containers[0]
	...
	postgresDeployment := kcAssert.FindGenericCreateAction(t, desiredState, model.PostgresqlDeployment(cr, false))
	postgresContainer := postgresDeployment.Ref.(*v13.Deployment).Spec.Template.Spec.Containers[0]
