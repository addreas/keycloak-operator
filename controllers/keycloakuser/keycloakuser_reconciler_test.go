...
func getDummyUserState(keycloak v1alpha1.Keycloak) *common.UserState {
...
func TestKeycloakUserReconciler_Reconcile(t *testing.T) {
	// given
	keycloak := v1alpha1.Keycloak{}
	realm := getDummyRealm()
	reconciler := NewKeycloakuserReconciler(keycloak, *realm)
	state := getDummyUserState(keycloak)
	user := getDummyUser()
